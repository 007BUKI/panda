...
├────────────────
├───➽ ${prefix}promote [@Marcação]
├───➽ ${prefix}promover [@Marcação]
├─➼ Promover a Adm
├────────────────
├───➽ ${prefix}demote [@Marcação]
├───➽ ${prefix}rebaixar [@Marcação]
├─➼ Rebaixar a Membro
├────────────────
├───➽ ${prefix}setname
├───➽ ${prefix}mudarnome
...
├─➼ Adicionar Membro
├────────────────
├───➽ ${prefix}kick [@Marcação]
├───➽ ${prefix}remover [@Marcação]
├─➼ Remover Membro
├────────────────
├───➽ ${prefix}listadmin
├─➼ Lista Dos Adms
...
├─➼ Faz Figurinha Led
├───➽ ${prefix}ttp
├─➼ Faz Figurinha Com Seu Nome
├───➽ ${prefix}nulis
├─➼ Escrever Nome No Caderno
├───➽ ${prefix}darkneon
├─➼ Seu Nome Em Neon
├───➽ ${prefix}candlemug
├─➼ Seu Nome Em Uma Xicara
├───➽ ${prefix}lovemsg
├─➼ Seu Nome Em Um Coração (Realista)
├───➽ ${prefix}mugflower
├─➼ Seu Nome Em Um Jarro De Flor
├───➽ ${prefix}narutobanner
├─➼ Seu Nome Com Foto Do Naruto
├───➽ ${prefix}paperonglass
├─➼ Seu Nome em um Papel
├───➽ ${prefix}romancetext
├─➼ Seu Nome Em Um Quadro
├───➽ ${prefix}shadowtext
├─➼ Foto linda Com Seu Nome
├───➽ ${prefix}coffeecup
├─➼ Seu Nome Em Uma Xicara De Café
├───➽ ${prefix}coffeecup2
├─➼ Seu Nome Em Uma Xicara De Café
├───➽ ${prefix}glowingneon
├─➼ Seu Nome Em Neon
...
├─➼ Seu Nome Na Madeira
├───➽ ${prefix}gplaybutton
├─➼ Seu Nome Na Placa De 1 Milhão Do Youtube
├───➽ ${prefix}splaybutton
├─➼ Seu Nome Na Placa De 100 Mil Do Youtube
├───➽ ${prefix}qrencode
├─➼ Seu Nome No QRCODE
├───➽ ${prefix}googletext
├─➼ Seu Nome No Google
...
├─➼ Seu Nome Em Glith
├───➽ ${prefix}lithgtext
├─➼ Seu Nome Em Luz Na Cidade
├───➽ ${prefix}crismes
├─➼ Seu Nome Em Neon Na Neve
├───➽ ${prefix}bneon
├─➼ Seu Nome Em Neon Verde
├───➽ ${prefix}matrix
├─➼ Seu No Matrix
├───➽ ${prefix}breakwall
├─➼ Seu Nome Em Uma Art
├───➽ ${prefix}dropwater
├─➼ Seu Nome Em Uma Janela (Chuva)
├───➽ ${prefix}leavest
├─➼ Seu Nome Em Folhas
├───➽ ${prefix}logobp
├─➼ Seu Nome Em Um Texto Rosa
└─────────
...
├───➽ ${prefix}tts
├─➼ Bot Manda Audio
├➻Exemplo: ${prefix}tts pt Olá, Diego!
└─────────
...
reply('Paciência, irmão, novamente escrevendo')
teks = body.slice(7)
				...
				case 'nsfw':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*「SUCESSO」ATIVA RECURSOS NSFW EM GRUPO*')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(nsfw))
...
case 'antidelete':
				...
				if (argz[1] == 'aktif') {
					if (isGroup) {
						...
						baby.sendMessage(from, `*Sucessos Ativam Grupo Antidelete!*`, MessageType.text)
					} else if (!isGroup) {
					...
					if (!isGroup) {
						...
						baby.sendMessage(from, `Para o grupo de uso *${prefix}antidelete aktif*`, MessageType.text)
					}
...
case 'adicionar':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					...
					try {
						...
						console.log('Error :', e)
						reply('Anjim que quer ser adicionado em privado, dahlah :)')
					}
					break
				case 'add':
                  if (isBanned) return reply(ind.baned())
...
case 'mining':
                      ...
                      reply(`porque Panda bot -x Good Bot oferece ${one}Xp >_<`)
                      }else{
